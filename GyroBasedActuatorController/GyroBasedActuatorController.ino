...
// kalman variables
float varSensor = 1e-5;  
float varMeasurement = 1e-8;
float Pc = 0.0;
float K = 0.0;
float P = 1.0;
float Zp = 0.0;
...
void loop(){
  ...
  // kalman process
  Pc = P + varMeasurement;
  K = Pc/(Pc + R);    // Kalman gain
  P = (1-K)*Pc+Q;     // Error coariance
  Zp = Xe;
  Xe = G*(GyroZ-Zp)+Xp;   // the kalman estimate of the sensor voltage
  ...
  // Print Data
  Serial.println(sqTarg);
  //Serial.print(",    ");
  Serial.println(0.03545*pos);
